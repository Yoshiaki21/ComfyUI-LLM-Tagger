...
- [ ] タイムアウト発生時に `X-Request-Id` ベースのキャンセルAPIが呼ばれ、`log.log` に成否が記録されること（13.1）
...
- キャンセルAPIのエンドポイントパス・リクエスト形式は、実装時に実サーバーで確認・確定させること（13.1参照）→ **11.5.0 では未提供であることを確認済み。定数 `LEMONADE_CANCEL_PATH` を用意して将来対応できる形にしてある（13.1の調査結果を参照）**
- 現状キャンセルできない以上、**タイムアウト後もサーバー側の生成はしばらく走り続ける可能性がある**。13.2 のパラメータ調整（`max_tokens` を段階的に絞る）が実質的な暴走対策の主軸となる
